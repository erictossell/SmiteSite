...
def live_match(dev_id, auth_key, session_id, time, player_name):    
    player_id = call("getplayer", dev_id, auth_key, session_id, time, "/"+player_name, "", "")[0]["Id"]
    if player_id == []:
        return "Not live!"
    print(player_id)
    ...
    if (player_status == "In Game"):
        ...
        for x in range(len(mdt)):
            ...
            if (mdt[x]['playerName'] != ''):
                ...
                match_data.append([mdt[x]['taskForce'],mdt[x]['GodName'],'','','','',''])
        ...
        for x in range(len(mx)):
            mx[x][1] = "https://web2.hirez.com/smite/god-icons/"+ mx[x][1].lower().replace(" ","-").replace("'","") +".jpg"
        return mx
...
def index():
    #return '<h1>O</h1>'
    live = live_match(dev_id, auth_key, session_id, time, name)
